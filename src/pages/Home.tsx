...
export default function Home() {
  ...
  return (
    <div className="bg-black min-h-screen text-white font-sans selection:bg-primary/30 pb-32">
      <div className="max-w-[1700px] mx-auto space-y-24 px-6 md:px-12 pt-8 animate-in fade-in duration-1000">
              ...
              >
                ...
                <div className="absolute inset-y-0 left-0 flex flex-col justify-center px-12 md:px-24 max-w-4xl space-y-10">
                  <div className="flex items-center gap-3">
                    <div className="bg-primary px-3 py-1 rounded-full"><Flame size={12} className="text-black" /></div>
                    <span className="text-[11px] font-black uppercase tracking-[0.4em] text-primary italic">Hot Release #{(heroIndex + 1)}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-6">
                    <button 
                      ...
                      className="bg-primary text-black h-24 px-16 rounded-[2rem] flex items-center gap-4 group hover:scale-[1.05] transition-all shadow-2xl shadow-primary/20"
                    >
            ...
            <div className="absolute bottom-16 right-24 hidden md:flex gap-4">
               {[0,1,2,3,4].map(idx => (
                 <button 
                  ...
                  className={`h-2 rounded-full transition-all duration-700 ${heroIndex === idx ? 'w-20 bg-primary' : 'w-4 bg-white/20 hover:bg-white/40'}`}
                 />
               ))}
            </div>
          </section>
        ...
        {continueWatching.length > 0 && (
          <section className="space-y-12">
            <div className="flex items-center gap-6 border-l-8 border-primary pl-10">
              <h2 className="text-4xl font-black italic uppercase tracking-tighter leading-none">Last <span className="text-primary">Session</span></h2>
                ...
                >
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-8 space-y-4">
                     ...
                     <div className="flex items-center justify-between">
                       <span className="text-[9px] font-black text-primary uppercase tracking-widest italic">EP {item.last_episode}</span>
                       <div className="w-16 h-1.5 bg-white/10 rounded-full overflow-hidden">
                          <div className="h-full bg-primary" style={{ width: `${Math.min(100, (item.progress_ms / 1440000) * 100)}%` }} />
                       </div>
                     </div>
                  </div>
                </Link>
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-4 gap-24">
          <div className="lg:col-span-3 space-y-32">
            ...
            <section className="space-y-14">
              <div className="flex items-center justify-between border-l-4 border-primary pl-10">
                <h2 className="text-5xl font-black italic uppercase tracking-tighter leading-none">Global <span className="text-primary">Network</span></h2>
                <Link to="/filter" className="text-[11px] font-black uppercase tracking-[0.4em] text-gray-600 hover:text-primary transition-all flex items-center gap-4 group">
                ...
                <div className="p-6 space-y-2">
                   {topAiring.slice(0, 10).map((anime, i) => (
                     <Link key={anime.mal_id} to={`/anime/${anime.mal_id}`} className="flex items-center gap-6 p-6 rounded-[2.5rem] hover:bg-white/5 transition-all group">
                        <span className="text-5xl font-black italic text-gray-900 group-hover:text-primary/20 transition-colors w-14 text-center">{i + 1}</span>
                        ...
                        <div className="flex-1 space-y-2">
                          <h4 className="text-[10px] font-black italic uppercase tracking-tight line-clamp-2 leading-tight group-hover:text-primary transition-all">{anime.title}</h4>
                          <div className="flex items-center gap-3">
